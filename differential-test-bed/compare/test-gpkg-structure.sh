...
#
#   differential-test-bed/compare/test-gpkg-structure.sh [source-archive.gpkg.tar]
#
...
# (exit 0) -- run `differential-test-bed/run/l1-merge-from-binpkg.sh` first for full
# coverage.
...
if [ -z "$SRC" ] || [ ! -f "$SRC" ]; then
  echo "test-gpkg-structure: SKIP -- no source archive (run differential-test-bed/run/l1-merge-from-binpkg.sh first)"
  exit 0
fi
SRC=$(realpath "$SRC")
