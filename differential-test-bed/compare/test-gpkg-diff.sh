...
# flipping with --mode.
#
#   differential-test-bed/compare/test-gpkg-diff.sh
#
# Exit 0 all good, 1 a case failed, 2 setup error. SKIPs (exit 0) when
# the L1 pkgcache has no suitable pair -- run
# `differential-test-bed/run/l1-merge-from-binpkg.sh differential-test-bed/atomlists/l1-porttest.txt` first.
