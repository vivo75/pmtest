#!/bin/bash
# L3 on VMs -- source-build parity: both package managers build+merge the
# same atom list FROM SOURCE into two fresh, identical guests under the
# determinism block, then the full installed tree + VDB are snapshotted
# (pulled back) and diffed on the host. Same grading as
# run/l3-source-parity.sh; reports l3-vm-*.
#
#   differential-test-bed/run/l3-source-parity-vm.sh [atomlist]
#
# Env:
#   L3_PM=both|portage|portuale  (default both; one-side iteration)
#   L3_CONTROL=1                 second portage guest (noise floor)
#   L3_BUILD_ARGS="--emptytree --oneshot --usepkg=n --color=n"
#   L3_DISTFILES                 default $LOGS_DIR/_l2-distfiles-vm
#   L3_TIMEOUT=28800             per-guest wall-clock cap
...
DISTFILES=${L3_DISTFILES:-$LOGS_DIR/_l2-distfiles-vm}
TIMEOUT=${L3_TIMEOUT:-28800}
RUN="l3-vm-$(timestamp)"
OUT="$LOGS_DIR/$RUN"
mkdir -p "$OUT" "$DISTFILES"
ln -sfn "$RUN" "$LOGS_DIR/l3-vm-latest"
...
run_pm() {  # <label> <portage|portuale>
  local label=$1 pm=$2 ip
  CURRENT_LABEL="porttest-l3-vm-$label-$$"
  ...
  set +e
  # NOTE: env assignments ride as `env K=V` argv, with the VALUE wrapped
  # in single quotes for the REMOTE shell: ssh joins its argv with
  # spaces, so local double quotes do not survive the trip and a value
  # with spaces (L3_BUILD_ARGS) would split into a bogus command.
  # Values never contain single quotes (flags and paths), so this is safe.
  timeout "$TIMEOUT" ssh $(ssh_opts) "${VM_SSH_USER}@${ip}" \
    env "DISTDIR=/distfiles" \
    "SNAPSHOT_PRUNE='$PM_REPO'" \
    "L3_SKIP_PORTAGE_UPGRADE='${L3_SKIP_PORTAGE_UPGRADE:-0}'" \
    "L3_BUILD_ARGS='${L3_BUILD_ARGS:---emptytree --oneshot --usepkg=n --color=n}'" \
    "L3_KEEP_TMP='${L3_KEEP_TMP:-0}'" \
...
{
  echo "# L3 source-build parity report (VM bed, fs=$VM_FS)"
  echo
...
ln -sfn "$RUN/l3-report.txt" "$LOGS_DIR/l3-vm-report.txt"
